<?
    session_start();
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="assets/css/style.css">
    <title>Document</title>
</head>
<body>
<?
    include('./assets/incl/header.php');
    if(isset($_SESSION['USER']) && $USER['role'] == 2){
    ?>
    <div class="profile">
        <div class="container">
            <div class="profile-content">
                <?
                if(isset($_GET['s2'])){
                    $get_s2 = $_GET['s2'];
                    $sql = "UPDATE add_zapic SET id_status = '2' WHERE id = '$get_s2'";
                    $link -> query($sql);
                    echo '<script>document.location.href="admin.php"</script>';
                }
                if(isset($_GET['s3'])){
                    $get_s3 = $_GET['s3'];
                    $sql = "UPDATE add_zapic SET id_status = '3' WHERE id = '$get_s3'";
                    $link -> query($sql);
                    echo '<script>document.location.href="admin.php"</script>';
                }
                ?>
                <p class="title-left-20">Профиль > все записи</p>
                <div class="zapici-content"><br>
                <?
                $sql = "SELECT * FROM add_zapic";
                $result = $link -> query($sql) -> fetchAll(2);
                foreach($result as $zapic){
                    $id_user = $zapic['id_user'];
                    $sql = "SELECT * FROM users WHERE id = '$id_user'";
                    $result = $link -> query($sql);
                    $user = $result -> fetch();

                    $id_master = $zapic['id_master'];
                    $sql = "SELECT * FROM masters WHERE id = '$id_master'";
                    $result = $link -> query($sql);
                    $master = $result -> fetch();
                    
                    $id_time = $zapic['id_time'];
                    $sql = "SELECT * FROM times WHERE id = '$id_time'";
                    $result = $link -> query($sql);
                    $time = $result -> fetch();

                    $id_status = $zapic['id_status'];
                    $sql = "SELECT * FROM status WHERE id = '$id_status'";
                    $result = $link -> query($sql);
                    $status = $result -> fetch();
                ?>
                    <div class="zapici-zapici">
                        <div>
                            <p class="text-bold">Запись № <?=$zapic['id']?></p>
                            <p class="text-16">Мастер: <?=$master['master']?></p>
                            <p class="text-16">Клиент: <?=$user['name']?></p>
                            <p class="text-16">Дата: <?=$zapic['date']?></p>
                            <p class="text-16">Время: <?=$time['time']?></p>
                        </div>
                        <div class="btn-status">
                            <p class="text-bold2">Статус: <?=$status['status']?></p>
                            <a href="?s2=<?=$zapic['id']?>" class="btn-2">Подтвердить</a>
                            <a href="?s3=<?=$zapic['id']?>" class="btn-2">Отменить</a>
                        </div>
                    </div><br>
                <?}?>
                </div>
            </div>
        </div>
    </div>
    <?}else{
        echo '';
    }
    ?>
</body>
</html>